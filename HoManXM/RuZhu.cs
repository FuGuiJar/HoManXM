...
namespace HoManXM
{
    public partial class RuZhu : Form
    {
        public RuZhu()
        {
            InitializeComponent();
        ...
        private void ChuShiHua()
        {
            ...
            string sql = $@"	select  r.room_id,l.roomLei_LeiBie,l.roomLei_jiaGe,t.roomTai_zhuangTai,YaJin.YaJin
	  from  room as r 
...
";
            ...
            if (reader.Read())
            {
                ...
                txtLeiXing.SelectedIndex = Convert.ToInt32(Fang) - 1;
                cboFanghao.Text =FangHao;

                txtJiaGe.Text = reader[2].ToString();
                txtDingJIn.Text = reader[4].ToString();
                //更改
...
//        }
	...
	where room.room_id = {Convert.ToInt32(cboFanghao.Text)}"; 
            SqlDataReader reader = DBHelper.ExecutReader(sql);
            if (reader.Read())
            {
